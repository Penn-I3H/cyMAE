...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, 
        ...
        self.feature_embed = nn.Parameter(torch.zeros(num_features, embed_dim-1))
        trunc_normal_(self.feature_embed, std=.02)
        ...
        if use_learnable_pos_emb:
            ...
            self.pos_embed = get_sinusoid_encoding_table(num_features, embed_dim)
    ...
    def forward_features(self, x):
        x = torch.concat((self.feature_embed.expand(x.size(0),self.num_features, self.embed_dim-1), x.view(x.size(0), x.size(1), 1)),axis=2)
...
class CNN1D(nn.Module):
    def __init__(self, in_features=30, out_features=2):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv1d(in_channels=1, out_channels=3, kernel_size=in_features), # [B, 1, 30] -> [B, 3, 1]
            nn.BatchNorm1d(3),
            nn.ReLU()
        )
        self.conv2 = nn.Sequential(
            nn.Conv1d(in_channels=3, out_channels=3, kernel_size=1), # [B, 3, 1] -> [B, 3, 1]
            nn.BatchNorm1d(3),
            nn.ReLU()
        )
...
@register_model
def DNN(pretrained=False, pretrained_cfg=False, **kwargs):
    model = Mlp(in_features=30, out_features=kwargs['num_classes'], act_layer=nn.ReLU)
    return model

@register_model
def CNN(pretrained=False, pretrained_cfg=False, **kwargs):
    model = CNN1D(in_features=30, out_features=kwargs['num_classes'])
    return model
